...
import type { GameStats, Topic } from "../types";
import {
  clearGameProgress,
  loadGameProgress,
  saveGameProgress,
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft" && currentHintIndex > 0) {
        ...
        topic &&
        currentHintIndex < topic.hints.length - 1
      ) {
        setCurrentHintIndex((i) => i + 1);
      }
    };
  ...
  async function handleSubmitGuess() {
    ...
    try {
      ...
      if (!moderationResult.appropriate) {
        setModerationError(
          moderationResult.message || "Please keep your guesses appropriate."
        );
        setSubmitting(false);
        return;
      }
  ...
  if (error || !topic) {
    return (
      <div className="min-h-screen bg-gradient-game flex flex-col items-center justify-center p-6 gap-6">
        <XCircle className="w-24 h-24 text-failure" />
      ...
      <div className="mb-8">
        ...
        <div className="w-full bg-white/10 rounded-full h-2.5 overflow-hidden shadow-inner">
          <div
            ...
            style={{
              width: `${((maxUnlockedHint + 1) / topic.hints.length) * 100}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center mb-8 px-2">
        <div className="w-full max-w-3xl">
          <div className="glass rounded-3xl p-6 sm:p-8 md:p-12 text-center shadow-2xl transform transition-all duration-300">
            <div className="mb-6">{getHintIcon(currentHint.type)}</div>
          ...
          <div className="flex items-center justify-center gap-6 mt-8">
            ...
            <button
              onClick={() =>
                setCurrentHintIndex((i) => Math.min(maxUnlockedHint, i + 1))
              }
      ...
      {!gameOver && (
        <div className="w-full max-w-3xl mx-auto space-y-4">
          ...
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && !submitting && handleSubmitGuess()
            }
